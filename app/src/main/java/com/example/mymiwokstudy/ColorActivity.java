...
public class ColorActivity extends AppCompatActivity {
    ...
    private int[] colorsImgRes, colorsAudioRes;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // get data
        colorsInEnglish = MiwokDict.getColorInEng();
        colorsInMiwok = MiwokDict.getColorInMiwok();
        colorsImgRes = MiwokDict.getColorImgRes();
        colorsAudioRes = MiwokDict.getColorAudioRes();

        // update display with data
        if (colorsInEnglish.length == colorsInMiwok.length) {
            for (int i = 0; i < colorsInEnglish.length; i++) {
                Word newWord = new Word(colorsInEnglish[i], colorsInMiwok[i], colorsImgRes[i]);
                newWord.setAudioResource(colorsAudioRes[i]);
                wordList.add(newWord);
            }
        } else {
            Log.w(LOG_TAG, "list length of colorsInEnglish does not match with colorsInMiwok");
        }
